...
class MultiObjectiveRewardMolecule(molecules_mdp.Molecule):
  ...
  def get_similarity(self, smiles):
    ...
    def check_diversity(self, similarity_score):
        diversity_check = 0
        if similarity_score >= 0.92:
            diversity_check = 1 - similarity_score
  ...
  def _reward(self):
    ...
    # calculate QED
    diversity_score = self.diversity_check(similarity_score)
    qed_value = QED.qed(mol)
    return similarity_score  * diversity_score * .gamma**(
