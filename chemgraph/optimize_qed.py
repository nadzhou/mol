...
from rdkit.Chem.MolSurf import PEOE_VSA10
FLAGS = flags.FLAGS
...
class QEDRewardMolecule(molecules_mdp.Molecule):
  ...
  def _reward(self):
    ...
    if molecule is None:
      return 0.0
    # pharm = Chem.Crippen.MolLogP(molecule)
    # pharm = Pharmacophore.Pharmacophore(molecule)
    pede = PEOE_VSA10(molecule)
    print(pede)
    return pede * self.discount_factor ** (self.max_steps - self.num_steps_taken)
...
class Pharm3dRewardMolecule(QEDRewardMolecule):
  """The molecule whose reward is the QED."""
  ...
  def _reward(self):
    ...
    if molecule is None:
      return 0.0
    AllChem.EmbedMolecule(molecule)  # gen 3d
    factory = Gobbi_Pharm2D.factory
    fp = Generate.Gen2DFingerprint(molecule, factory, dMat=Chem.Get3DDistanceMatrix(molecule))
    fp = np.array(fp)
    print(np.unique(fp))
    return fp * self.discount_factor ** (self.max_steps - self.num_steps_taken)
